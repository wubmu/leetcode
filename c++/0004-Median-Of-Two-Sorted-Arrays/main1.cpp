...
2)  a[k/2 -1] > b[k/2 - 1] , ˵�� b[0, k/2 -1] ��Χ�ڵ�Ԫ��һ����c[k/2 -1]С�� ���Բ�����b[0,k/2 - 1]
3)  a[k/2 -1] < b[k/2 -1] ���ͣ�2���෴

�ݹ����������

1) a[k/2 - 1] == b[k/2 - 1] return a[k/2 -1]
2) a��bΪ��ʱ, b[k/2 -1]��a[k/2-1]
3) k==1ʱ��return min(a[0], b[0])
    ...
    int getK(const vector<int>& nums1,int start1, int end1, vector<int>& nums2,int start2, int end2, int k){
       ...
       // printf("start %d\n %d ",start1, min(k/2, len1));
        ...
        if(nums1[index1] > nums2[index2])
            return getK(nums1, start1, end1, nums2, index2+1, end2, k - min(k/2, len2));

        return getK(nums1, index1+1 , end1, nums2, start2, end2 , k - min(k/2, len1));
